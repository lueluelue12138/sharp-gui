...
function Get-UpdateCompatibilityInfo {
    ...
    if ($info.SchemaVersion -ne $script:SupportedManifestSchemaVersion) {
        Fail "不支持 update-manifest.json schemaVersion=$($info.SchemaVersion)，当前仅支持 $($script:SupportedManifestSchemaVersion)"
    }
    if ($info.UpdateProtocolRevision -ne $script:SupportedUpdateProtocolRevision) {
        Fail "不支持 updateProtocolRevision=$($info.UpdateProtocolRevision)，当前仅支持 $($script:SupportedUpdateProtocolRevision)"
    }
    ...
    if (-not (Test-VersionAtLeast -Actual $script:MinGitVersion -Minimum $info.MinimumGitVersion)) {
        Fail "固定 MinGit $($script:MinGitVersion) 低于兼容清单最低版本 $($info.MinimumGitVersion)"
    }
...
function Get-SourceRevision {
    ...
    try {
        ...
        Fail "无法调用构建机 Git 解析源代码 revision: $($_.Exception.Message)"
    }
...
- **Stable** 指最新正式 GitHub Release；**Latest** 指 ``main`` 最新提交，测试程度低于 Stable。
- 自动更新只替换兼容的受管应用代码，并支持回滚到上一个 revision。
